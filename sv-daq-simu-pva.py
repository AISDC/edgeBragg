'''
made by Sinisa which controls/scheduls the frame sending 
to aovide frame loss due to instant high throughput
behaves like UDP but it suppose to be TCP/IP based,
still puzzling here. 
'''
...
class daqSimuEPICS:
    ...
    def __init__(self, h5, daq_freq, nf, nx, ny, runtime, channel_name, start_delay):
        self.arraySize = None
        self.delta_t = 1.0/daq_freq
        self.runtime = runtime
        
        if h5 is None:
            self.frames = np.random.randint(0, 256, size=(nf, nx, ny), dtype=np.uint16)
        else:
    ...
    def frame_producer(self, extraFieldsPvObject=None):
        for frame_id in range(0, self.n_generated_frames):
            ...
            nda['value'] = {'ushortValue': self.frames[frame_id].flatten()}
...
if __name__ == '__main__':
    ...
    parser.add_argument('-nx', type=int, default=2048, help='number of pixels in x dimension (does not apply if h5 file is given)')
    parser.add_argument('-ny', type=int, default=2048, help='number of pixels in y dimension (does not apply if h5 file is given)')
    parser.add_argument('-nf', type=int, default=240, help='number of frames to generate')
    ...
    daq = daqSimuEPICS(h5=args.ifn, daq_freq=args.fps, nf=args.nf, nx=args.nx, ny=args.ny, runtime=args.rt, channel_name=args.cn, start_delay=args.sd)
