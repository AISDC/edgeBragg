...
class pvaClient:
    def __init__(self, nth=1):
        self.psz = 15
        self.patch_tq = queue.Queue(maxsize=-1)
        mdl_fn = 'models/fc16_8_4_2-sz15.onnx'
        self.infer_engine = inferBraggNN(mbsz=256, onnx_mdl=mdl_fn, patch_tq=self.patch_tq)
    ...
    def frame_process(self, ):
        while self.thr_exit == 0:
            ...
            logging.info("[%.3f] %d patches cropped from frame %d, %.3fms/frame, %d peaks are too big; "\
                         "%d out of %d frames processed so far, %d patches pending infer" % (\
                         time.time(), patches.shape[0], frm_id, elapse, big_peaks, self.frames_processed, \
                         self.recv_frames, self.patch_tq.qsize()))
